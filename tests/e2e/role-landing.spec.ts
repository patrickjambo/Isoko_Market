...
import { registerWithIntentUI, otpLoginUI, logoutUI, uniqueEmail } from './helpers';
...
test('employer: register → Post-a-Job, and login → employer dashboard', async ({ page }) => {
  ...
  await registerWithIntentUI(page, { email, name: 'E2E Employer', intent: 'hire' });
  await expect(page, 'hire registration lands on Post-a-Job').toHaveURL(/\/en\/jobs\/new/);

  await logoutUI(page);
  await otpLoginUI(page, email);
...
test('job seeker: register → CV builder, and login → jobs home', async ({ page }) => {
  ...
  await registerWithIntentUI(page, { email, name: 'E2E Seeker', intent: 'find_work' });
