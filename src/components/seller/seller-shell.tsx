...
const STORAGE_KEY = 'isoko_seller_sidebar_collapsed';

type NavItem = { href: string; label: string; icon: LucideIcon; exact?: boolean; badge?: number };

/**
 * Collapsible, persisted seller sidebar (Section 1) — Home · My Listings ·
 * Messages · Wallet · Profile, with a prominent always-reachable "+ Sell".
 * Mirrors the admin shell: icon-rail when collapsed, slide-over drawer on mobile.
 */
    ...
    <>
      <div className="p-2">
        ...
        >
          ...
          {!mini && <span>{t('sell')}</span>}
        </Link>
      </div>
      <nav className="flex flex-col gap-1 p-2" aria-label="Seller">
        {items.map((item) => {
